        ...
        sock.ev.on('messages.update', async (updates) => {
            try {
                ...
                for (const { key, update } of updates) {
                    if (!update?.message && update?.messageStubType !== 1 && !update?.messageStubType) continue;
                    const isRevoke = update?.message === null || update?.messageStubType === 1;
                    if (!isRevoke || !store) continue;
                    const original = store.getMessage(key.remoteJid, key.id);
                    if (!original) continue;
                    const target = OWNER_CLEAN_JID;
                    const from = key.remoteJid;
                    const text = original.message?.conversation
                        || original.message?.extendedTextMessage?.text
                        || '[non-text message]';
                    try {
                        await sock.sendMessage(target, { text: `🗑️ *Anti-Delete*\nA message was deleted in ${from}\n\n${text}` });
                    } catch {}
                }
            } catch {}
        });
    ...
    try {
        ...
        if (command) {
            try {
                ...
                await command.execute(sock, msg, args, currentPrefix, { OWNER_NUMBER: OWNER_CLEAN_NUMBER, OWNER_JID: OWNER_CLEAN_JID, OWNER_LID, BOT_NAME, VERSION, isOwner: () => jidManager.isOwner(msg), jidManager, store, statusDetector, updatePrefix: updatePrefixImmediately, getCurrentPrefix, rateLimiter, memberDetector, isPrefixless, commands, commandCategories });
